...
export class Options
{
    ...
    setRenderer()
    {        
        // Force WebGL - comment out WebGPU check for compatibility
        const element = this.element.querySelector('.js-renderer')
        element.classList.remove('is-success')
        element.classList.add('is-danger')

        const text = element.querySelector('span')
        text.textContent = 'WebGL (forced)'

        const tooltip = element.querySelector('.js-tooltip')
        tooltip.innerHTML = /* html */`Using WebGL for compatibility - enable WebGPU in chrome://flags/ if desired`
